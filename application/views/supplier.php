										...
										<tbody>
											...
											foreach ($read as $d) { ?>
												<tr>
													...
													<td><?= $d->nama_supplier ?></span></td>
													<td class="text-center">+62<?= $d->no_hp ?></span></td>
													<td><?= $d->alamat ?></td>
													<td class="text-center">
														<div class="btn-group">
															<a class="btn bg-green" onclick="return ubah(`<?= $d->id ?>`, `<?= $d->nama_supplier ?>`, `<?= $d->no_hp ?>`, `<?= $d->alamat ?>`)"><i class="fas fa-edit fa-fw"></i></a>
					...
					<div class="modal-body">
						...
						<span id="modal-body-update-or-create">
							<div class="form-group">
								<label>Nama</label>
								<input id="nama_supplier" name="nama_supplier" type="text" class="form-control" maxlength="25" placeholder="Nama Supplier">
							</div>
							<div class="form-group">
								<label>No. HP</label>
...
<script>
	...
	function tambah() {
		$('#Modal').modal('show');
		$('#formulir').attr('action', '<?= base_url('master/supplier/tambah'); ?>');
	...
	function ubah(id, nama_supplier, no_hp, alamat) {
		$('#Modal').modal('show');
		$('#formulir').attr('action', '<?= base_url('master/supplier/ubah'); ?>');
	...
	function hapus(id, nama_supplier) {
		$('#Modal').modal('show');
		$('#formulir').attr('action', '<?= base_url('master/supplier/hapus'); ?>');
